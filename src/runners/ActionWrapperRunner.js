...
/**
 * Handles the overall wrapper flow using Witness for attestation
 */
class ActionWrapperRunner {
  constructor() {
  ...
  async run() {
    try {
      ...
      core.info('Witness wrapper run completed successfully');
    } catch (error) {
      core.setFailed(`Witness wrapper action failed: ${error.message}`);
      process.exit(1);
    }
  }
...
module.exports = ActionWrapperRunner;
